...
const ExportAboveTeachersButton = ({ user, departmentId, departmentName }) => {
    ...
    const exportToPDF = async () => {
        try {
            ...
            const aboveTeachers = await getAboveTeachers(departmentId);
            ...
            const docDefinition = {
                content: [
                    ...
                    { text: '\n' },
                    { text: `BẢNG THỐNG KÊ GIÁO VIÊN VƯỢT QUÁ 25% SỐ TIẾT CƠ BẢN - TỔ ${departmentName.toUpperCase()}`, style: 'title' },
                    ...
                    { text: '\n' },
                    {
                        table: {
                            ...
                            body: [
                                ...
                                ...aboveTeachers.map((teacher, index) => [
                                    index + 1,
                                    teacher.name,
            ...
            pdfMake.createPdf(docDefinition).download(`BaoCaoGiaoVienThua_${departmentName}.pdf`);
        } catch (error) {
            console.error('Error exporting to PDF:', error);
            toast.error('Có lỗi xảy ra khi xuất báo cáo giáo viên thừa tiết');
        }
    };
