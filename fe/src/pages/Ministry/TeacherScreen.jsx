...
const TeacherScreen = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
          console.log("Đang gửi dữ liệu sau để tạo giáo viên:", excelData);
          ...
          if (response.invalidTeachers && response.invalidTeachers.length > 0) {
            ...
            const errorMessages = response.invalidTeachers.map(item => 
              `${item.teacher.email || 'Unknown'}: ${item.errors.join(', ')}`
            ).join('\n');
            toast.error(`Một số giáo viên không hợp lệ:\n${errorMessages}`);
          }
      
          if (response.errors && response.errors.length > 0) {
            console.log("Lỗi từ server:", response.errors);
            const serverErrorMessages = response.errors.map(err => `${err.email}: ${err.message}`).join('\n');
            toast.error(`Một số giáo viên không thể được tạo:\n${serverErrorMessages}`);
          }
      
          if (response.createdTeachers && response.createdTeachers.length > 0) {
            console.log("Giáo viên đã được tạo:", response.createdTeachers);
